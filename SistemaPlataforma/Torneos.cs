...
namespace SistemaPlataforma
{
    public partial class Torneos : Form
    {
        ...
        private void Torneos_Load(object sender, EventArgs e)
        {
            ...
            if ((TorneosDisponibles.fechaInicioCompetenciaSeleccionada <= DateTime.Now && TorneosDisponibles.fechaFinCompetenciaSeleccioada > DateTime.Now) || (TorneosDisponibles.fechaInicioCompetenciaSeleccionada >= DateTime.Now && TorneosDisponibles.fechaFinCompetenciaSeleccioada > DateTime.Now))
            {
                oCompetencia.State = new DisponibleCompetenciaState(this);
            }
            if (TorneosDisponibles.fechaInicioCompetenciaSeleccionada < DateTime.Now && TorneosDisponibles.fechaFinCompetenciaSeleccioada > DateTime.Now)
            {
        ...
        private void Torneos_MouseDown(object sender, MouseEventArgs e)
        {
            ReleaseCapture();
        ...
        {
            TorneosDisponibles frmTorneosDisponibles = new TorneosDisponibles();
            frmTorneosDisponibles.Show();
            this.Hide();
        }
        ...
        private void btnSalir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                SqlCommand comando1 = new SqlCommand("Select * from Inscripciones inner join Equipos on Inscripciones.idEquipo = Equipos.idEquipo where idCompetencia = " + TorneosDisponibles.idCompetenciaSeleccionada + " and Equipos.capitan = '" + Login.nicknameUsuario + "'", SqlCon1);
                SqlCon1.Open();
        ...
        private void btnPartidosFinalizados_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                //Establecemos la conexion, el comando y ejecutamos el mismo
                SqlCon.ConnectionString = @"Data Source=(LocalDb)\MSSQLLocalDB;Initial Catalog=PlataformaProduccion;Integrated Security=True";
                SqlCommand comando = new SqlCommand("select Partidos.idPartido, Partidos.fecha, Partidos.equipoA, Partidos.equipoB, Partidos.resultado, Partidos.juego, Partidos.arbitro, Competencias.nombreCompetencia from Partidos inner join Competencias on Partidos.idCompetencia = Competencias.idCompetencia where nombreCompetencia = '" + TorneosDisponibles.nombreCompetenciaSeleccionada + "' order by fecha desc", SqlCon);
                SqlCon.Open();
                SqlDataAdapter sqlDataAdap = new SqlDataAdapter(comando);
                DataTable dtRecord = new DataTable();
                sqlDataAdap.Fill(dtRecord);
